...
bool IRrecv::decodeHash(decode_results *results) {
  ...
  for (uint16_t i = 1; i < results->rawlen - 2; i++) {
    ...
    char buffer[12]; // Temporary buffer to store the string representation of each integer
    sprintf(buffer, "%d", value); // Convert integer to string
    strcat(result, buffer);
    
    // Serial.print(value);
    // Serial.print(" ");
    // total = total + 2^i + value;
  ...
  char *found_one=NULL;
  uint16_t value = 0;
  found_one= strstr(result,substr);
  if (found_one != NULL){
    value = 1;
  }
  Serial.println();
